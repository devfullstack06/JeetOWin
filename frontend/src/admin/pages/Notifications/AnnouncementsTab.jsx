...
function CreateAnnouncementModal({ open, onClose, options, onSaved }) {
  ...
  const uploadImages = async (files) => {
    ...
    const res = await fetch("/api/admin/announcements/upload-images", {
      method: "POST",
      headers: token ? { Authorization: `Bearer ${token}` } : {},
      body: fd,
    });
  ...
  const submit = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setErrorText(data?.message || "Failed to create announcement.");
        setSaving(false);
        return;
      }
      onSaved?.();
      ...
      setErrorText(e?.message || "Failed to create announcement.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="jw-adminUsersModalOverlay jw-adminUsersModalOverlay--belowHeader">
      <div className="jw-adminUsersModal jw-adminUsersModal--scrollable jw-adminUsersModal--ngForm">
        <div className="jw-adminUsersModal__header jw-adminAnnModalHeader">
          <div className="jw-adminUsersModal__title">Create announcement</div>
...
export default function AnnouncementsTab() {
  ...
  const fetchList = useCallback(() => {
    ...
    fetch(`/api/admin/announcements?${qs}`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    })
      .then((r) => r.json())
      .then((data) => {
        if (!Array.isArray(data.items)) {
          setRows([]);
          setTotal(0);
          setErrorText(data?.message || "Failed to load announcements.");
          return;
        }
        setRows(data.items);
      ...
      .catch(() => {
        setRows([]);
        setTotal(0);
        setErrorText("Failed to load announcements.");
      })
  ...
  const openAudience = async (id) => {
    const token = localStorage.getItem("token") || "";
    const res = await fetch(`/api/admin/announcements/${id}/audience`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    });
    const data = await res.json().catch(() => ({}));
    setDetailsModal({
  ...
  const openSeen = async (id) => {
    const token = localStorage.getItem("token") || "";
    const res = await fetch(`/api/admin/announcements/${id}/seen`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    });
    const data = await res.json().catch(() => ({}));
    setDetailsModal({
          ...
          <table className="jw-adminTable">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="jw-adminEmpty">No announcements found</td></tr>
              ) : rows.map((r) => (
                <tr key={r.id}>
                  <td>{r.id}</td>
                  <td>{r.title}</td>
