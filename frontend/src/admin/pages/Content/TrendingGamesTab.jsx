...
function TrendingGamesModal({ open, title, form, saving, errorText, file, onFile, previewUrl, imageInfo, onChange, onCancel, onConfirm, requireImage }) {
  if (!open) return null;
...
export default function TrendingGamesTab() {
  ...
  const fetchItems = useCallback(() => {
    ...
    fetch("/api/admin/trending-games", { headers: token ? { Authorization: `Bearer ${token}` } : {} })
      .then((res) => res.json())
      .then((data) => {
        if (ignore) return;
      ...
      .catch(() => { if (!ignore) setItems([]), setErrorText("Unable to load trending games."); })
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch("/api/admin/trending-games", { method: "POST", headers: token ? { Authorization: `Bearer ${token}` } : {}, body: fd });
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (editFile) {
        ...
        res = await fetch(`/api/admin/trending-games/${editing.id}`, { method: "PATCH", headers: token ? { Authorization: `Bearer ${token}` } : {}, body: fd });
      } else {
        res = await fetch(`/api/admin/trending-games/${editing.id}`, {
          method: "PATCH",
  ...
  const handleDelete = (row) => {
    if (!window.confirm(`Delete trending item "${row.name}"?`)) return;
    const token = localStorage.getItem("token") || "";
    fetch(`/api/admin/trending-games/${row.id}`, { method: "DELETE", headers: token ? { Authorization: `Bearer ${token}` } : {} })
      .then((res) => {
  ...
  const editPreviewSrc = editFile ? editPreview : (editing?.imagePath || "");
  ...
  return (
    <>
      ...
      >
        <AdminFilterField label="Trending Games">
          <span className="jw-adminUsersModal__hint">Manage tiles shown in the home page Trending Games carousel.</span>
        </AdminFilterField>
      </AdminFilterBar>
        ...
        <table className="jw-adminTable">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="jw-adminEmpty">No trending items yet.</td></tr>
            ) : (
              items.map((r) => (
                <tr key={r.id}>
                  <td>{r.sortOrder}</td>
