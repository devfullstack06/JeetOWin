...
const INBOX_IMAGE_MAX_FILE_BYTES = Math.floor(2.5 * 1024 * 1024);
const INBOX_IMAGE_MAX_TOTAL_BYTES = 25 * 1024 * 1024;
...
async function refreshDueInboxMessages(connLike = pool) {
...
exports.getAdminInboxFilterOptions = async (req, res) => {
  try {
    ...
    if (e.code === "ER_NO_SUCH_TABLE") {
      return res.status(503).json({
        message: "Inbox dependencies are missing. Run database/migration_inbox_messages.sql",
      });
    }
    console.error("[inbox/options]", e);
    return res.status(500).json({ message: "Failed to load inbox filter options." });
  }
};

exports.postAdminInboxAffiliateCountPreview = async (req, res) => {
  try {
    ...
    console.error("[inbox/affiliate-count]", e);
    return res.status(500).json({ message: "Failed to preview affiliate audience." });
  }
};

/**
 * POST /api/admin/inbox/member-count-preview
 ...
 */
exports.postAdminInboxMemberCountPreview = async (req, res) => {
  try {
    const groupIds = [
      ...new Set(
    ...
    console.error("[inbox/member-count-preview]", e);
    return res.status(500).json({ message: "Failed to preview member count." });
  }
};

exports.getAdminInboxMessages = async (req, res) => {
  try {
    await refreshDueInboxMessages(pool);
    const id = String(req.query.id || "").trim();
    const username = String(req.query.username || "").trim().toLowerCase();
    const band = toSafeBand(req.query.band);
    const groupId = req.query.groupId ? Number(req.query.groupId) : null;
    const statusRaw = String(req.query.status || "").trim().toLowerCase();
    const audienceModeRaw = String(req.query.audienceMode || "").trim().toLowerCase();
    ...
    if (audienceModeRaw === "all" || audienceModeRaw === "custom" || audienceModeRaw === "affiliates") {
      where.push("a.audience_mode = ?");
      params.push(audienceModeRaw);
    }
    if (band && Number.isInteger(groupId) && groupId > 0) {
      where.push(
        "EXISTS (SELECT 1 FROM inbox_audience_rows aar WHERE aar.inbox_message_id = a.id AND aar.band = ? AND aar.group_id = ?)"
      );
      params.push(band, groupId);
    }
    if (username) {
      where.push(
        "EXISTS (SELECT 1 FROM inbox_recipients ar INNER JOIN users u ON u.id = ar.user_id WHERE ar.inbox_message_id = a.id AND LOWER(u.username) LIKE ?)"
      );
      params.push(`%${username}%`);
    }
    const whereSql = where.length ? `WHERE ${where.join(" AND ")}` : "";
    const [[countRow]] = await pool.query(
      `SELECT COUNT(*) AS total FROM inbox_messages a ${whereSql}`,
      params
    );
    ...
    const [rows] = await pool.query(
      ...
      SELECT
        ...
        (
          SELECT COUNT(*)
          FROM inbox_reads ar2
          WHERE ar2.inbox_message_id = a.id
        ) AS seen_count
      FROM inbox_messages a
      ${whereSql}
    ...
    if (e.code === "ER_NO_SUCH_TABLE") {
      return res.status(503).json({
        message: "Inbox tables are missing. Run database/migration_inbox_messages.sql",
        items: [],
        total: 0,
      });
    }
    console.error("[inbox/list]", e);
    return res.status(500).json({ message: "Failed to load inbox messages." });
  }
};

exports.getAdminInboxMessageById = async (req, res) => {
  try {
    ...
    if (!a) return res.status(404).json({ message: "Inbox message not found." });
    const [rows] = await pool.query(
      `SELECT band, group_id FROM inbox_audience_rows WHERE inbox_message_id = ? ORDER BY id ASC`,
      [a.id]
    );
    const grouped = [];
    const keyed = new Map();
    ...
    const [images] = await pool.query(
      `SELECT path, sort_order FROM inbox_images WHERE inbox_message_id = ? ORDER BY sort_order ASC, id ASC`,
      [a.id]
    );
    const [excluded] = await pool.query(
      `
      SELECT u.id AS userId, u.username
      FROM inbox_excluded_users ex
      INNER JOIN users u ON u.id = ex.user_id
      WHERE ex.inbox_message_id = ?
    ...
    return res.status(500).json({ message: "Failed to load inbox message." });
  }
};

exports.createAdminInboxMessage = async (req, res) => {
  ...
  const imagePaths = imagePathsRaw
    .map((x) => String(x || "").trim())
    .filter((x) => x.startsWith("/uploads/inbox/"))
    .slice(0, 10);
  ...
  if (imagePaths.length) {
    ...
    for (const webPath of imagePaths) {
      const rel = webPath.startsWith("/uploads/inbox/")
        ? webPath.slice("/uploads/inbox/".length)
        : "";
      if (!rel || rel.includes("..") || rel.includes("/") || rel.includes("\\")) {
        return res.status(400).json({ message: "Invalid inbox image path." });
      }
      ...
      if (!st.isFile()) {
        return res.status(400).json({ message: "Invalid inbox image file." });
      }
      if (st.size > INBOX_IMAGE_MAX_FILE_BYTES) {
        return res.status(400).json({ message: "Each image must be 2.5MB or smaller." });
      }
      totalBytes += st.size;
      if (totalBytes > INBOX_IMAGE_MAX_TOTAL_BYTES) {
  ...
  try {
    ...
    await refreshDueInboxMessages(conn);
    ...
    if (audienceMode === "all") {
      ...
      if (includedIds.length) {
        const allowed = new Set(affiliateIds);
        const picked = includedIds.filter((id) => allowed.has(id));
        if (!picked.length) {
          await conn.rollback();
          return res.status(400).json({
            message: "Selected users must be active affiliate accounts.",
          });
        }
        recipientsSet = new Set(picked);
      } else {
    ...
    for (const row of audienceRows) {
      for (const gid of row.groupIds) {
        await conn.query(
          `INSERT INTO inbox_audience_rows (inbox_message_id, band, group_id) VALUES (?, ?, ?)`,
          [msgId, row.band, gid]
        );
      }
    }
    for (const uid of excludedIds) {
      await conn.query(
        `INSERT INTO inbox_excluded_users (inbox_message_id, user_id) VALUES (?, ?)`,
        [msgId, uid]
      );
    }
    for (let i = 0; i < imagePaths.length; i += 1) {
      await conn.query(
        `
        INSERT INTO inbox_images (inbox_message_id, sort_order, path, original_name, mime, size_bytes)
        VALUES (?, ?, ?, NULL, NULL, NULL)
        `,
        [msgId, i + 1, imagePaths[i]]
      );
    }
    ...
    console.error("[inbox/create]", e);
    return res.status(500).json({ message: "Failed to create inbox message." });
  } finally {
    conn.release();
  }
};

exports.deleteAdminInboxMessage = async (req, res) => {
  try {
    const id = String(req.params.id || "").trim();
    const [del] = await pool.query(`DELETE FROM inbox_messages WHERE public_id = ?`, [id]);
    if (!del.affectedRows) return res.status(404).json({ message: "Inbox message not found." });
    ...
    console.error("[inbox/delete]", e);
    return res.status(500).json({ message: "Failed to delete inbox message." });
  }
};

exports.getAdminInboxAudience = async (req, res) => {
  try {
    const id = String(req.params.id || "").trim();
    const [[a]] = await pool.query(
      `SELECT id FROM inbox_messages WHERE public_id = ? LIMIT 1`,
      [id]
    );
    if (!a) return res.status(404).json({ message: "Inbox message not found." });
    ...
    const [users] = await pool.query(
      `
      SELECT u.id AS userId, u.username
      FROM inbox_recipients ar
      INNER JOIN users u ON u.id = ar.user_id
      WHERE ar.inbox_message_id = ?
      ORDER BY u.username ASC
      `,
      [a.id]
    );
    ...
    console.error("[inbox/audience]", e);
    return res.status(500).json({ message: "Failed to load audience details." });
  }
};

exports.getAdminInboxSeenBy = async (req, res) => {
  try {
    const id = String(req.params.id || "").trim();
    const [[a]] = await pool.query(
      `SELECT id FROM inbox_messages WHERE public_id = ? LIMIT 1`,
    ...
    console.error("[inbox/seen]", e);
    return res.status(500).json({ message: "Failed to load seen-by users." });
  }
};

exports.getClientInboxUnreadCount = async (req, res) => {
  try {
    await refreshDueInboxMessages(pool);
    ...
    const [[row]] = await pool.query(
      `
      SELECT COUNT(*) AS cnt
      FROM inbox_recipients rc
      INNER JOIN inbox_messages a ON a.id = rc.inbox_message_id
      LEFT JOIN inbox_reads rd ON rd.inbox_message_id = a.id AND rd.user_id = rc.user_id
      WHERE rc.user_id = ?
        AND a.status = 'sent'
    ...
    console.error("[client/inbox/unread-count]", e);
    return res.status(500).json({ error: "Failed to load unread count." });
  }
};

exports.getClientInboxMessages = async (req, res) => {
  try {
    await refreshDueInboxMessages(pool);
    const userId = Number(req.user?.userId || 0);
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
    ...
    if (ids.length) {
      ...
      const [imgRows] = await pool.query(
        ...
        FROM inbox_images ai
        INNER JOIN inbox_messages a ON a.id = ai.inbox_message_id
    ...
    console.error("[client/inbox]", e);
    return res.status(500).json({ error: "Failed to load inbox messages." });
  }
};

exports.markClientInboxMessageRead = async (req, res) => {
  try {
    const userId = Number(req.user?.userId || 0);
    const publicId = String(req.params.id || "").trim();
    if (!userId || !publicId) return res.status(400).json({ error: "Invalid request." });
    const [[a]] = await pool.query(`SELECT id FROM inbox_messages WHERE public_id = ? LIMIT 1`, [
      publicId,
    ]);
    if (!a) return res.status(404).json({ error: "Inbox message not found." });

    const [[isRecipient]] = await pool.query(
      `SELECT 1 AS ok FROM inbox_recipients WHERE inbox_message_id = ? AND user_id = ? LIMIT 1`,
      [a.id, userId]
    );
    ...
    await pool.query(
      `
      INSERT INTO inbox_reads (inbox_message_id, user_id, read_at)
    ...
    console.error("[client/inbox/read]", e);
    return res.status(500).json({ error: "Failed to mark inbox message as read." });
  }
};
