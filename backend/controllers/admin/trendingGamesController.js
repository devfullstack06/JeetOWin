...
const { UPLOADS_TRENDING_GAMES } = require("../../middleware/uploadTrendingGamesImage");
...
function uniqueName(prefix = "trending") {
  const slug = String(prefix).replace(/[^a-z0-9-]/gi, "-").replace(/-+/g, "-").slice(0, 20) || "item";
  return `tg-${slug}-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`;
}
...
function saveImage(buffer, mimetype, base) {
  ...
  const full = path.join(UPLOADS_TRENDING_GAMES, baseName);
  try {
    fs.mkdirSync(UPLOADS_TRENDING_GAMES, { recursive: true });
    fs.writeFileSync(full, buffer);
  } catch (e) {
    console.error("save trending game image:", e.message);
    return { error: "Failed to save image." };
  }
  return { imagePath: `/uploads/trending-games/${baseName}` };
}

function deleteImage(storedPath) {
  if (!storedPath || typeof storedPath !== "string" || !storedPath.startsWith("/uploads/trending-games/")) return;
  const base = path.basename(storedPath);
  const full = path.join(UPLOADS_TRENDING_GAMES, base);
  try {
    if (fs.existsSync(full)) fs.unlinkSync(full);
  } catch (e) {
    console.error("delete trending game image:", e.message);
  }
}
...
exports.getPublicTrendingGamesItems = async (req, res) => {
  try {
    let rows = [];
    try {
      [rows] = await pool.query(
        `SELECT id, name, image_path, link_url, open_in_new_tab, sort_order
         FROM trending_games_items
         WHERE is_active = 1
    ...
    console.error("getPublicTrendingGamesItems:", err);
    return res.status(500).json({ message: "Failed to load trending games." });
  }
};

exports.getAdminTrendingGamesItems = async (req, res) => {
  try {
    ...
    if (err.code === "ER_NO_SUCH_TABLE") {
      return res.status(503).json({ message: "Trending games table missing. Run migration_trending_games_items.sql." });
    }
    console.error("getAdminTrendingGamesItems:", err);
    return res.status(500).json({ message: "Failed to load trending games." });
  }
};

exports.createAdminTrendingGamesItem = async (req, res) => {
  try {
    ...
    if (sortOrder === null) {
      const [[r]] = await pool.query("SELECT COALESCE(MAX(sort_order), 0) + 1 AS n FROM trending_games_items").catch(() => [{ n: 1 }]);
    ...
    const [result] = await pool.query(
      `INSERT INTO trending_games_items (name, image_path, link_url, open_in_new_tab, sort_order, is_active)
       VALUES (?, ?, ?, ?, ?, ?)`,
    ...
    const [[row]] = await pool.query(
      `SELECT id, name, image_path, link_url, open_in_new_tab, sort_order, is_active
       FROM trending_games_items WHERE id = ?`,
      [insertId]
    );
    ...
    console.error("createAdminTrendingGamesItem:", err);
    return res.status(500).json({ message: "Failed to create trending item." });
  }
};

exports.updateAdminTrendingGamesItem = async (req, res) => {
  try {
    ...
    if (updates.length) {
      params.push(id);
      await pool.query(`UPDATE trending_games_items SET ${updates.join(", ")} WHERE id = ?`, params);
    }
    ...
    console.error("updateAdminTrendingGamesItem:", err);
    return res.status(500).json({ message: "Failed to update trending item." });
  }
};

exports.deleteAdminTrendingGamesItem = async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!Number.isFinite(id) || id <= 0) return res.status(400).json({ message: "Invalid id." });
    const [rows] = await pool.query("SELECT id, image_path FROM trending_games_items WHERE id = ?", [id]);
    if (!rows.length) return res.status(404).json({ message: "Item not found." });
    await pool.query("DELETE FROM trending_games_items WHERE id = ?", [id]);
    ...
    console.error("deleteAdminTrendingGamesItem:", err);
    return res.status(500).json({ message: "Failed to delete trending item." });
  }
};
